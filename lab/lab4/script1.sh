...
echo "ilość pustych linii: $(grep '^$' -r $1 | wc -l)" 1
...
echo "ilość linii nie zawierających cyfr 0-9 oraz podkreślników_: $(grep '[0-9_]' -rv $1 | wc -l)"
...
echo "ilość linii zawierających liczby hexadecymalne: $(grep '0x[A-F0-9]+' -r $1 )"
...
echo "ilość linii zakończonych trójliterowym rozszerzeniem pliku: $(grep -r '\.[^.\{3\}]$' | wc -l)"
